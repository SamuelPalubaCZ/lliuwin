...
The image comes from signed Ubuntu noble/noble-updates/noble-security repositories, with `ubuntu-desktop`, `linux-generic` and `initramfs-tools`. GNOME Initial Setup is intended to create the first personal account. No shared password is embedded. Native Python 2.7.18 is retained only for this transition release.
